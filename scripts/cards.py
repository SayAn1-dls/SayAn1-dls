...
THEMES = {
    "dark": {"bg":"#0d1117","border":"#30363d","title":"#39d353","text":"#c9d1d9","muted":"#8b949e","value":"#e6edf3","accent":"#39d353"},
    "light": {"bg":"#ffffff","border":"#d0d7de","title":"#1a7f37","text":"#1f2328","muted":"#57606a","value":"#1f2328","accent":"#1a7f37"},
}
LANG_COLOR = {
    "JavaScript":"#f1e05a","TypeScript":"#3178c6","Python":"#3572A5","HTML":"#e34c26","CSS":"#563d7c",
    "C++":"#f34b7d","C":"#555555","Java":"#b07219","Go":"#00ADD8","Rust":"#dea584","Shell":"#89e051",
...
def fetch_contributions(user,token):
    ...
    try:
        ...
        print(f"contributions unavailable: {e}",file=sys.stderr); return None
...
def render_stats(user,stats,theme):
    c=THEMES[theme]; pad=22; tiles=[(v,k) for k,v in stats]; cols=3
    rows=(len(tiles)+cols-1)//cols; rh,W=46,480
    H=pad+52+(rows-1)*rh+17+pad; out=[]; tw=(W-2*pad)/cols
    out=[f'<text x="{pad}" y="{pad+14}" font-size="15" font-weight="700" fill="{c["title"]}">{esc(user)}</text>',
         f'<text x="{W-pad}" y="{pad+14}" font-size="11" text-anchor="end" fill="{c["muted"]}">at a glance</text>',
         f'<line x1="{pad}" y1="{pad+26}" x2="{W-pad}" y2="{pad+26}" stroke="{c["border"]}"/>']
    top=pad+52
    for i,(value,label) in enumerate(tiles):
        cx=pad+(i%cols)*tw; cy=top+(i//cols)*rh
        out.append(f'<text x="{cx:.0f}" y="{cy:.0f}" font-size="23" font-weight="700" fill="{c["value"]}">{esc(value)}</text>')
        out.append(f'<text x="{cx:.0f}" y="{cy+17:.0f}" font-size="10.5" fill="{c["muted"]}">{esc(label)}</text>')
    return frame(W,H,c,"".join(out),f"{user} GitHub statistics")

def render_repo(repo,theme):
    c=THEMES[theme]; W,H=420,132; pad=18; out=[]
    out.append(icon(ICON_REPO,pad,pad,15,c["muted"]))
    out.append(f'<text x="{pad+22}" y="{pad+12}" font-size="14.5" font-weight="700" fill="{c["title"]}">{esc(repo["name"])}</text>')
    desc=repo.get("description") or "No description yet."
    for i,line in enumerate(wrap(desc,11.5,W-2*pad,3)):
        out.append(f'<text x="{pad}" y="{pad+36+i*16}" font-size="11.5" fill="{c["text"]}">{esc(line)}</text>')
    fy=H-pad-2; x=pad
    if repo.get("language"):
        col=LANG_COLOR.get(repo["language"],c["muted"])
        out.append(f'<circle cx="{x+5}" cy="{fy-4}" r="5" fill="{col}"/>')
        out.append(f'<text x="{x+15}" y="{fy}" font-size="11" fill="{c["muted"]}">{esc(repo["language"])}</text>')
    ...
    for path,count in ((ICON_STAR,repo.get("stars",0)),(ICON_FORK,repo.get("forks",0))):
        ...
        out.append(f'<text x="{x+17}" y="{fy}" font-size="11" fill="{c["muted"]}">{count}</text>')
...
def main(argv=None):
    ...
    tiles=[("Total stars",f"{stars:,}"),("Public repos",f"{user['public_repos']:,}"),("Followers",f"{user['followers']:,}")]
    contrib=fetch_contributions(args.user,token)
    if contrib:
        total,current,longest=contrib
        tiles+=[("Contributions (1y)",f"{total:,}"),("Current streak",f"{current:,}"),("Longest streak",f"{longest:,}")]
    for theme in ("dark","light"):
        dest=args.out/f"card-stats-{theme}.svg"
        dest.write_text(render_stats(args.user,tiles,theme),encoding="utf-8")
    print(f"wrote card-stats-*.svg")
    if not args.projects.exists(): return
    ...
    for entry in wanted:
        ...
        if not src: print(f"!! {entry['repo']} not found, skipped"); continue
        card={"name":src["name"],"description":entry.get("description") or src.get("description"),
              "language":entry.get("language") or src.get("language"),
              "stars":src["stargazers_count"],"forks":src["forks_count"]}
        for theme in ("dark","light"):
